Ext.define('NgcpCsc.view.pages.callblocking.incoming.IncomingController', {
    ...
    renderBarrNumber: function(value, meta, record) {
        if (record.get('enabled') === false) {
            return Ext.String.format('<div style="text-decoration: line-through;font-size: 18px;">{0}</div>', value);
        } else {
            return Ext.String.format('<div style="font-size: 18px;">{0}</div>', value);
        }
    },
    ...
    onEnabledClicked: function (event, el) {
        console.log(event); // constructor
        console.log(el); // DOM element
        // TODO: Get class from event
        if (el.toString().indexOf('trash') > -1) {
            console.log('delete');
        } else if (el.toString().indexOf('toggle') > -1){
            console.log('toggle');
        };
