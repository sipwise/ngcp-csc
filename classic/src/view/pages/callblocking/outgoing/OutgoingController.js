Ext.define('NgcpCsc.view.pages.callblocking.outgoing.OutgoingController', {
    ...
    addNewNumber: function () {
        ...
        var store = Ext.getStore('CallBlockingOutgoing');
        var newId = store.getCount() + 1;
        var newNumber = vm.get('outgoing.new_out_number');
        ...
        if (invalidCheck !== 1) {
            if (newNumber.length === 0) {
                this.fireEvent('showmessage', false, Ngcp.csc.locales.callblocking.please_enter_number[localStorage.getItem('languageSelected')]);
            } else {
            newNumber = '+' + newNumber.join('');
            store.add({ "id": newId, "block_out_list": newNumber, "enabled": true });
        ...
        vm.set('outgoing.new_out_number', '');
    },

    clickModeButton: function () {
        var vm = this.getViewModel();
        var blockAllowMode = vm.get('outgoing.block_in_mode');
