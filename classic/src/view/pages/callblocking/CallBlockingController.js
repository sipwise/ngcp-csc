Ext.define('NgcpCsc.view.pages.callblocking.CallBlockingController', {
        ...
        switch (storeType) {
            case 'block_in_list':
                ...
                options["update"] = [Ext.create('NgcpCsc.model.Patch', {
                    ...
                    "op": "add",
                    "path": "/block_in_mode",
                    "value": vm.get('incoming_block_mode') === 'on' ? true : false
                })];
                break;
            case 'block_out_list':
                Ext.each(store.getRange(), function(record) {
                ...
                options["update"] = [Ext.create('NgcpCsc.model.Patch', {
                    ...
                    "op": "add",
                    "path": "/block_out_mode",
                    "value": vm.get('outgoing_block_mode') === 'on' ? true : false
                })];
                break;
            default: // privacy
                options["update"] = [Ext.create('NgcpCsc.model.Patch', {
    ...
    setVm: function(store, data) {
        var vm = this.getViewModel();
        vm.set('incoming_block_mode', data.get('block_in_mode') === true ? 'on' : 'off');
        vm.set('outgoing_block_mode', data.get('block_out_mode') === true ? 'on' : 'off');
        vm.set('privacy_block_mode', data.get('clir') === true ? 'on' : 'off');
