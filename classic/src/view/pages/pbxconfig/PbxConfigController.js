Ext.define('NgcpCsc.view.pages.pbxconfig.PbxConfigController', {
    ...
    abortEdit: function () {
        ...
        var addPbxActive = vm.get('add_pbx_active');
        // TODO: This is causing issues, as it also results in new card not
        // being deleted when switching modules
        if (vm.get('add_pbx_active')) { return; };
        switch (storeName) {
            case 'Groups':
            case 'Seats':
            case 'Devices':
                switch (addPbxActive) {
                    case true:
                        console.log('addPbxActive');
                        // TODO: Toggle icons at bottom of card back to edit and no cancel
                        // var el = document.getElementById('edit' + storeName.slice(0, -1) + '-' + newId);
                        var store = Ext.getStore(storeName);
                        // var elClassList = el.firstChild.classList;
                        var selectedRow = store.findRecord('id', recId);
