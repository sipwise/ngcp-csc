Ext.define('NgcpCsc.view.pages.PbxConfigGrid', {
    ...
    // TODO: 0. Consider mapping this out in a flow chart (or other way) to
    // clearly define how to handle the id-to-row relationship for tasks below
    // TODO: 1. Change addSeat() to only toggle in same row id
    // TODO: 2. Add onclick listeners for toggling only specific fields
    // TODO: 3. Add onchange listeners to all fields, and autosave in controller
    // TODO: 4. Refactor controller functions in the end, once all grunt work
    // is done
            ...
            getAdditionalData: function(data, idx, record) {
                ...
                switch (window.location.hash) {
                    ...
                    var content = '<div class="card-wrapper hidden pointer" id="seatsCard-' + record.get('id') + '">' +
                        '<div class="card-data-row"><span></span><b>' + Ngcp.csc.locales.filters.extension[localStorage.getItem('languageSelected')] + '</b>: <span class="pbx-data-value">' + record.get('extension') + '</span><input class="pbx-data-field" type="text" id="seatsExtensionInput-' + record.get('id') + '" /></div>' +
                        '<div class="card-data-row"><span></span><b>' + Ngcp.csc.locales.filters.groups[localStorage.getItem('languageSelected')] + '</b>: <span class="pbx-data-value">' + record.get('groups') + '</span><input class="pbx-data-field" type="text" id="seatsGroupsInput-' + record.get('id') + '" /></div>' +
                        '<div class="card-data-row"><span></span><b>' + Ngcp.csc.locales.filters.numbers[localStorage.getItem('languageSelected')] + '</b>: <span class="pbx-data-value">' + record.get('numbers') + '</span><input class="pbx-data-field" type="text" id="seatsNumbersInput-' + record.get('id') + '" /></div>' +
                        '<div class="card-data-row"><span></span><b>' + Ngcp.csc.locales.filters.phone_devices[localStorage.getItem('languageSelected')] + '</b>: <span class="pbx-data-value">' + record.get('phone_devices') + '</span><input class="pbx-data-field" type="text" id="seatsPhoneDevicesInput-' + record.get('id') + '" /></div>' +
