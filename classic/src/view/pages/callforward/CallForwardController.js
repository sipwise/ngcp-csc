Ext.define('NgcpCsc.view.pages.callforward.CallForwardController', {
    ...
    createNewSourceset: function(name, mode, callback) {
        Ext.Ajax.request({
    ...
    createNewSource: function(response, opts) {
    ...
    cfSourcesetStoreLoaded: function(store, data) {
        ...
        var arrayOfModels = [];
        if (data.getData()._embedded == undefined) {
            this.createNewSourceset('List A', 'whitelist');
            this.createNewSourceset('List B', 'whitelist');
            store.load();
        } else {
    ...
    getDestStoresCount: function(storesPrefix) {
        var onlineStore = Ext.getStore(storesPrefix + 'CallForwardOnline');
        var offlineStore = Ext.getStore(storesPrefix + 'CallForwardOffline');
        var busyStore = Ext.getStore(storesPrefix + 'CallForwardBusy');
        return onlineStore.getCount() + offlineStore.getCount() + busyStore.getCount();
    },
        ...
        Ext.each(cfTabPanels, function(tabP) {
            Ext.each(sourcesets, function(sourceset, index) {
                ...
                if (Ext.ComponentQuery.query("[name=" + tabP._tabId + '-tab-' + strippedSourcesetName + "]").length < 1) {
                    ...
                    var formula = {};
                    var destStoresCount = me.getDestStoresCount(tabP._firstPrefixes[index + 1] + tabP._secondprefix);
                    formula['from-' + strippedSourcesetName + "-" + tabP._tabId + '-title-formula'] = function(get) {
                        var key = 'from-' + strippedSourcesetName + "-" + tabP._tabId + '-title';
                        return this.get(key) + ' (' + destStoresCount +')';
                    }
                    vm.setFormulas(formula);
                }
            });
        });
    },
    ...
    writeNewSourceToStore: function(grid) {
        ...
        if (record && record.get('source') !== ' ' && record.get('source') !== '') {
            ...
            this.createNewSourceset(store.sourcesetName, 'whitelist', this.createNewSource);
        }
    },
