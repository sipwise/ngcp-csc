                ...
                case true:
                    recordsToSend.push({
                        ...
                        records: [{
                            ...
                            "destination": data.simple_destination,
                            "priority": data.priority,
                            "timeout": data.ring_for
                        }]
                    });
                    break;
                case false:
        ...
        Ext.each(cfTabPanels, function(tabP) {
            Ext.each(sourcesets, function(sourceset, index) {
                ...
                if (Ext.ComponentQuery.query("[name=" + tabP._tabId + '-tab-' + strippedSourcesetName + "]").length < 1) {
                    ...
                    tabP.add({
                        title: Ngcp.csc.locales.callforward.from[localStorage.getItem('languageSelected')] + sourcesetName,
                        ...
                        items: [
                            Ext.create('NgcpCsc.view.pages.callforward.CallForwardMainForm', {
                                _title: vm.get(tabP._tabId + strippedSourcesetName + '-title'),
                                _isEverybody: false,
