    ...
    clickSegmentedButton: function (el) {
        ...
        Ext.Ajax.request({
            ...
            success: function(response, opts) {
                ...
                for (storeIndex in storesArray) {
                    var storeName = storesArray[storeIndex];
                    var store = Ext.getStore(storeName);
                    // DONE: 1a. Remove all records
                    store.removeAll();
                    for (node in combinationStore) {
                        if (me.checkIndex(storeName, node)) {
                            // DONE: 1b. Extract the matching combination from the json
                            // - check that we can access the combinations
                            // - the logged data is now record sets that need to be loaded
                            // into the store (possibly broken down into records first)
                            console.log(obj.data[0][node]);
