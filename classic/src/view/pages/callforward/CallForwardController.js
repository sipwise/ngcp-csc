            ...
            if (record.get('destination') === 'own phone') {
                ...
                Ext.Ajax.request({
                    ...
                    success: function(response) {
                        var decodedResponse = Ext.decode(response.responseText);
                        var mappings = decodedResponse['cfu'];
                        mappings.push({
                        ...
                        // TODO Need to also remove mapping for cft path ("path": "/cft"), so we
                        // need to create a separate function that handles that first, with
                        // array filter helper function:
                        // mappings = mappings.filter(function( obj ) {
