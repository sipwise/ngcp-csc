Ext.define('NgcpCsc.view.pages.callforward.CallForwardController', {
    ...
    cfSourcesetBeforeSync: function(store, options) {
        // Overriding Ext JS proxy and using ajax request instead as we are
        // using different url params for PATCH compared to GET
        delete options['destroy'];
        delete options['create'];
        delete options['update'];
    ...
    cfTimesetBeforeSync: function(store, options) {
        ...
        Ext.each(store.getRange(), function(rec, index) {
            ...
            if (!rec.get('day')) { // periods wit no day are not going to be saved
                ...
                multiDayPeriod = multiDayPeriod || daysMapping[rec.get('day')];
            }
        });
        Ext.Ajax.request({
    ...
    isLastCftDestinationInStore: function(store) {
    ...
    updateMappings: function(json, subscriber) {
        Ext.Ajax.request({
