Ext.define('NgcpCsc.view.common.gridfilters.GridFiltersController', {
    ...
    applyCallFilters: function(record) {
        ...
        var retVal = true;
        if (fromFilter.length > 1 && new Date(record.get('start_time')) < new Date(fromFilter) ||
            toFilter.length > 1 && new Date(record.get('start_time')) >  new Date(toFilter).setHours(23,59,59,999) ||
