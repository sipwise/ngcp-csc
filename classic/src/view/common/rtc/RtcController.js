Ext.define('NgcpCsc.view.common.rtc.RtcController', {
    ...
    showRtcPanel: function(record, action, switchVideoOn) {
        ...
        switch (action) {
            ...
            case 'phoneComposer':
                if(vm.get('connected')){
                    this.fireEvent('showmessage', false, 'Please terminate current call before starting a new one.');
                    return;
                }
    ...
    toggleCall: function(btn) {
        if (btn.pressed) { // this can be also done with vm.get('callEnabled')
    ...
    sendFax: function() {
        ...
        if(faxForm.isValid()){
            ...
            this.fireEvent('showmessage', false, 'Invalid or missing fields.');
        }
