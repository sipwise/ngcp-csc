Ext.define('NgcpCsc.view.common.rtc.RtcController', {
    ...
    showRtcPanel: function(record, action, switchVideoOn) {
        ...
        switch (action) {
            case 'startCall':
            case 'startVideoCall':
                var number = record.get('name') || record.get('source_cli') || record.get('caller');
                ...
                vm.set('uid', record.get('uid') || null);
    ...
    toogleChat: function(btn) {
        ...
        var contact = contactsStore.findRecord('uid', vm.get('uid'));
